...
export const LeadCard = ({ lead, onVerify, onPreQual, onAdvanceKYC, onApproveKYC, onSetup, onConvert, onDetail, onDelete, onRiskScan, onScheduleMeeting }) => {
  ...
  const getRiskBadge = (lead) => {
    ...
    const cfg = s >= 700 ? { bg: 'bg-green-900/40', text: 'text-green-400', label: t('otc.trustworthy') }
      : s >= 500 ? { bg: 'bg-emerald-900/40', text: 'text-emerald-400', label: t('otc.high') }
      : s >= 300 ? { bg: 'bg-yellow-900/40', text: 'text-yellow-400', label: t('otc.review') }
      : s >= 150 ? { bg: 'bg-orange-900/40', text: 'text-orange-400', label: t('otc.low') }
      : { bg: 'bg-red-900/40', text: 'text-red-400', label: t('otc.risk') };
    return (
        ...
        <div className="flex items-center gap-5">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-4 text-sm text-gray-400">
              ...
              <span className="flex items-center gap-1"><TrendingUp size={14} />{t(`otc.sourceLabels.${lead.source}`) || lead.source?.replace('_', ' ') || 'Website'}</span>
            </div>
          </div>

          <div className="text-right flex-shrink-0 mr-4">
            <p className="text-gray-400 text-xs uppercase mb-1">{t('otc.estimatedVolume')}</p>
          ...
          <div className="flex items-center gap-2 flex-shrink-0" onClick={e => e.stopPropagation()}>
            ...
            {lead.status === 'pre_qualified' && (
              <Button size="icon" variant="outline" className="w-10 h-10 border-indigo-500/30 text-indigo-400 hover:bg-indigo-900/20"
                onClick={e => { e.stopPropagation(); onAdvanceKYC(lead.id); }} title={t('otc.advanceToKYC')} data-testid={`kyc-lead-${lead.id}`}><ChevronRight size={18} /></Button>
            )}
            {lead.status === 'kyc_pending' && (
              <Button size="icon" variant="outline" className="w-10 h-10 border-yellow-500/30 text-yellow-400 hover:bg-yellow-900/20"
                onClick={e => { e.stopPropagation(); onApproveKYC(lead.id); }} title={t('otc.approveKYC')} data-testid={`approve-kyc-${lead.id}`}><CheckCircle size={18} /></Button>
            )}
            {lead.status === 'kyc_approved' && (
              <Button size="icon" variant="outline" className="w-10 h-10 border-cyan-500/30 text-cyan-400 hover:bg-cyan-900/20"
                onClick={e => { e.stopPropagation(); onSetup(lead); }} title={t('otc.operationalSetup')} data-testid={`setup-lead-${lead.id}`}><Settings size={18} /></Button>
            )}
            {lead.status === 'setup_pending' && (
              <Button size="icon" variant="outline" className="w-10 h-10 border-green-500/30 text-green-400 hover:bg-green-900/20"
                onClick={e => { e.stopPropagation(); onConvert(lead.id); }} title={t('otc.convertToClient')} data-testid={`convert-lead-${lead.id}`}><UserPlus size={18} /></Button>
            )}
            {!lead.trustfull_data && (
              <Button size="icon" variant="outline" className="w-10 h-10 border-blue-500/30 text-blue-400 hover:bg-blue-900/20"
                onClick={e => { e.stopPropagation(); onRiskScan(lead.id); }} title={t('otc.riskScan')} data-testid={`risk-scan-${lead.id}`}><Shield size={18} /></Button>
            )}
            <Button size="icon" variant="outline" className="w-10 h-10 border-blue-500/30 text-blue-400 hover:bg-blue-900/20"
              onClick={e => { e.stopPropagation(); onScheduleMeeting(lead); }} title={t('otc.scheduleMeeting')} data-testid={`schedule-meeting-${lead.id}`}><Video size={18} /></Button>
            <Button size="icon" variant="outline" className="w-10 h-10 border-gold-500/30 text-gold-400 hover:bg-gold-900/20"
              onClick={e => { e.stopPropagation(); onDetail(lead); }} title={t('otc.viewDetails')} data-testid={`view-lead-${lead.id}`}><Edit size={18} /></Button>
            {lead.status !== 'active_client' && (
              <Button size="icon" variant="outline" className="w-10 h-10 border-red-500/30 text-red-400 hover:bg-red-900/20"
                onClick={e => { e.stopPropagation(); onDelete(lead.id); }} title={t('otc.deleteLead')} data-testid={`delete-lead-${lead.id}`}><Trash2 size={18} /></Button>
            )}
          </div>
        </div>
      </CardContent>
