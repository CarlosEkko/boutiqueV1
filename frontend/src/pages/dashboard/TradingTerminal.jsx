...
const TradingTerminal = () => {
  ...
  const handleSelectPair = (base, quote = 'USDT') => {
    setSelectedSymbol(`${base}${quote}`);
  };

  const getDisplayPair = () => {
    // Extract base/quote from symbol
    const fiatQuotes = ['EUR', 'USD', 'BRL', 'AED'];
    const cryptoQuotes = ['USDT', 'BTC', 'ETH', 'BNB'];
    for (const q of [...fiatQuotes, ...cryptoQuotes]) {
      if (selectedSymbol.endsWith(q)) {
        return { base: selectedSymbol.slice(0, -q.length), quote: q };
      }
    }
    return { base: selectedSymbol, quote: '' };
  };

  const displayPair = getDisplayPair();
  ...
  return (
    <div className="h-[100vh] flex flex-col bg-[#0b0e11] text-white overflow-hidden" data-testid="trading-terminal">

      {/* Top ticker bar */}
      <div className="h-10 border-b border-zinc-800 flex items-center px-3 gap-6 text-xs shrink-0 bg-[#0b0e11]">
        <div className="flex items-center gap-2">
          <span className="text-white font-semibold text-sm">{displayPair.base}/{displayPair.quote}</span>
        </div>
        {ticker && (
          <>
