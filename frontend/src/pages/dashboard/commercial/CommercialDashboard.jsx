            ...
            <CardContent className="p-0">
              {commissions.length > 0 ? commissions.map((c, i) => (
                <div key={c.id} className={`flex items-center justify-between px-5 py-3 ${i > 0 ? 'border-t border-zinc-800/50' : ''}`}>
                  <div>
                    <p className="text-white text-sm">{c.period_label}</p>
                    <p className="text-gray-500 text-xs">Base: {formatCurrency(c.base_commission)} + Bonus: {formatCurrency(c.bonuses)}</p>
                  </div>
                  <div className="flex items-center gap-3">
...
const SellersTab = ({ token }) => {
  ...
  if (selectedSeller) {
    return <SellerDetail sellerId={selectedSeller} token={token} onBack={() => setSelectedSeller(null)} />;
  }
