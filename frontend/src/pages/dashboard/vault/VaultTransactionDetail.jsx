...
const VaultTransactionDetail = () => {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6" data-testid="vault-detail-page">
      <button onClick={() => navigate('/dashboard/vault')} className="text-zinc-500 hover:text-zinc-200 flex items-center gap-1.5 text-sm transition-colors">
        <ArrowLeft size={16} /> Voltar ao Vault
      </button>

      {/* Hero Header */}
      <div className="relative rounded-2xl bg-zinc-900 border border-zinc-800/60 p-8 overflow-hidden">
        ...
        <div className="relative flex items-center justify-between">
          <div className="flex items-center gap-5">
            <div className={`w-16 h-16 rounded-2xl flex items-center justify-center ${
              ...
              <Send size={28} className={tx.status === 'completed' ? 'text-emerald-400' : tx.status === 'rejected' ? 'text-rose-400' : 'text-amber-400'} />
            </div>
            <div>
          ...
          <div className="flex items-center gap-3">
            {tx.status === 'pending_signatures' && timeLeft && (
              <Badge className="bg-zinc-800 text-zinc-400 border border-zinc-700 px-4 py-2 rounded-full">
            ...
            {canCancel && (
              <Button variant="outline" size="sm" onClick={handleCancel} className="border-rose-500/30 text-rose-400 hover:bg-rose-500/10 rounded-full" data-testid="vault-cancel-btn">
        ...
        <div className="mt-8 text-center">
          <p className="text-zinc-500 text-xs uppercase tracking-[0.2em] mb-2">Montante</p>
          <p className="text-5xl font-mono font-extralight text-zinc-50 tracking-tighter">
            -{new Intl.NumberFormat('pt-PT', { minimumFractionDigits: 2, maximumFractionDigits: 6 }).format(tx.amount)}
          </p>
          <p className="text-zinc-400 text-sm mt-1">{tx.asset} <span className="text-zinc-600">({tx.network})</span></p>
        </div>
      </div>

      <div className="grid grid-cols-5 gap-6">
        {/* Left: Details + Actions (2 cols) */}
        <div className="col-span-2 space-y-4">
          <Card className="bg-zinc-900 border-zinc-800/50">
            <CardContent className="p-6 space-y-4">
              <h3 className="text-xs font-semibold uppercase tracking-[0.2em] text-zinc-500">Detalhes</h3>
              <Row label="Token" value={`${tx.asset} (${tx.network})`} />
              <Row label="Origem" value={tx.source_wallet} />
              ...
              <Row label="Endereço" value={
                <div className="flex items-center gap-1">
                  ...
                  <button onClick={() => copyText(tx.destination_address)} className="text-zinc-600 hover:text-zinc-300"><Copy size={11} /></button>
                </div>
              } />
              <Row label="Order" value={<span className="font-mono text-xs">{tx.order_number}</span>} />
              {tx.notes && <Row label="Notas" value={<span className="text-zinc-400 text-xs">{tx.notes}</span>} />}
              {tx.execution?.tx_hash && (
                <Row label="TxHash" value={
                  <div className="flex items-center gap-1">
                    <span className="font-mono text-xs text-emerald-400 truncate max-w-[180px]">{tx.execution.tx_hash}</span>
                    <button onClick={() => copyText(tx.execution.tx_hash)} className="text-zinc-600 hover:text-zinc-300"><Copy size={11} /></button>
                  </div>
                } />
              )}
            </CardContent>
            ...
            <Card className="bg-zinc-900 border-amber-500/20 shadow-[0_0_30px_rgba(245,158,11,0.05)]" data-testid="vault-sign-card">
              <CardContent className="p-6 space-y-4">
                <h3 className="text-xs font-semibold uppercase tracking-[0.2em] text-amber-400 flex items-center gap-2">
                  <Lock size={14} /> A sua assinatura
                </h3>
                <Input value={comment} onChange={e => setComment(e.target.value)} placeholder="Comentário (opcional)..."
                  className="bg-zinc-800/50 border-zinc-700/50 text-white text-sm rounded-xl" data-testid="vault-sign-comment" />
                <Button onClick={handleSign} disabled={signing} data-testid="vault-sign-btn"
                  className="w-full bg-amber-500 text-zinc-950 hover:bg-amber-400 rounded-full font-medium shadow-[0_0_20px_rgba(245,158,11,0.15)] transition-all hover:shadow-[0_0_30px_rgba(245,158,11,0.25)]">
                  <ShieldCheck size={18} className="mr-2" /> {signing ? 'A assinar...' : 'Assinar Transação'}
                </Button>
                <Button onClick={handleReject} disabled={signing} variant="ghost" data-testid="vault-reject-btn"
                  className="w-full text-zinc-500 hover:text-rose-400 rounded-full">
          ...
          {tx.activity_log?.length > 0 && (
            <Card className="bg-zinc-900 border-zinc-800/50">
              <CardContent className="p-6">
                <h3 className="text-xs font-semibold uppercase tracking-[0.2em] text-zinc-500 mb-4 flex items-center gap-2"><FileText size={14} /> Activity Log</h3>
                <div className="space-y-2 max-h-[200px] overflow-y-auto">
                  {tx.activity_log.map((log, i) => (
                    <div key={i} className="flex items-start gap-2 text-xs py-1.5 border-b border-zinc-800/30 last:border-0">
        ...
        <div className="col-span-3">
          <Card className="bg-zinc-900 border-zinc-800/50">
            <CardContent className="p-6">
              <h3 className="text-xs font-semibold uppercase tracking-[0.2em] text-zinc-500 mb-8">Process</h3>
              ...
              <div className="space-y-0">
                {steps.map((step, idx) => {
                  ...
                  return (
                    <div key={step.key} className="flex gap-5" data-testid={`vault-step-${step.key}`}>
                      <div className="flex flex-col items-center w-5">
                        <div className={`w-4 h-4 rotate-45 rounded-[3px] border-2 flex-shrink-0 transition-all duration-500 ${st.diamond}`} />
                        {!isLast && <div className={`w-0.5 flex-1 min-h-[48px] transition-all duration-500 ${st.line}`} />}
                      </div>
                      <div className="pb-8 flex-1">
                        ...
                        {step.key === 'signatures' && step.details && (
                          <div className="mt-3 space-y-3">
                            <p className="text-xs text-zinc-500">
                              {step.details.required} assinatura(s) necessária(s), {step.details.signed} assinada(s)
                            </p>
                            <div className="space-y-2">
                              {(step.details.signers || []).map((s, i) => (
                                <div key={i} className={`flex items-center justify-between p-3 rounded-xl border transition-all ${
                                  ...
                                  <div className="flex items-center gap-2">
                                    {s.comment && <span className="text-zinc-600 text-[10px] max-w-[120px] truncate">"{s.comment}"</span>}
                                    <Badge className={`rounded-full text-[10px] px-2.5 ${
                                      ...
                                      {s.status === 'signed' ? 'Signed' : s.status === 'rejected' ? 'Rejected' : 'Pending'}
                                    </Badge>
                                  </div>
                                </div>
                              ))}
                        ...
                        {step.key === 'risk_check' && step.status === 'completed' && (
                          <p className="mt-2 text-xs text-emerald-400/70">{step.details?.details || 'All checks passed'}</p>
                        )}
                        ...
                        {step.key === 'completed' && step.status === 'completed' && step.timestamp && (
                          <p className="mt-2 text-xs text-emerald-400/70">{new Date(step.timestamp).toLocaleString('pt-PT')}</p>
                        )}
                      </div>
                    </div>
                  );
...
const Row = ({ label, value }) => (
  <div className="flex justify-between items-start py-1 border-b border-zinc-800/30 last:border-0">
