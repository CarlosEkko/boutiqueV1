...
const VaultSignatories = () => {
  const { token } = useAuth();
  const { t } = useLanguage();
  const roleLabels = { admin: t('vaultSignatories.roleAdmin', 'Admin'), signer: t('vaultSignatories.roleSigner', 'Signatário'), viewer: t('vaultSignatories.roleViewer', 'Visualizador') };
  const roleColors = {
  ...
  const addSignatory = async () => {
    if (!newSigner.email) { toast.error(t('vaultSignatories.emailRequired', 'Email é obrigatório')); return; }
    try {
      await axios.post(`${API_URL}/api/vault/signatories`, newSigner, { headers });
      toast.success(t('vaultSignatories.added', 'Signatário adicionado'));
      setShowAddDialog(false);
      setNewSigner({ email: '', name: '', role: 'signer' });
      fetchAll();
    } catch (err) { toast.error(err.response?.data?.detail || t('vaultSignatories.errorGeneric', 'Erro')); }
  };

  const removeSignatory = async (id) => {
    if (!window.confirm(t('vaultSignatories.confirmRemove', 'Remover este signatário?'))) return;
    try {
      await axios.delete(`${API_URL}/api/vault/signatories/${id}`, { headers });
      toast.success(t('vaultSignatories.removed', 'Removido'));
      fetchAll();
    } catch { toast.error(t('vaultSignatories.errorRemove', 'Erro ao remover')); }
  };
  ...
  const saveSettings = async () => {
    ...
    try {
      ...
      toast.success(t('vaultSignatories.saved', 'Configurações guardadas'));
    } catch (err) { toast.error(err.response?.data?.detail || t('vaultSignatories.errorGeneric', 'Erro')); }
  ...
  if (loading) return <div className="text-center py-20 text-zinc-500">{t('vaultSignatories.loading', 'A carregar...')}</div>;
    ...
    <div className="max-w-4xl mx-auto space-y-8" data-testid="vault-signatories-page">
        ...
        <div className="relative flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-light text-zinc-50 flex items-center gap-3">
              ...
              {t('vaultSignatories.title', 'Signatários')}
            </h1>
            <p className="text-zinc-400 text-sm mt-2">{t('vaultSignatories.subtitle', 'Gerir os seus signatários autorizados e configurações de threshold')}</p>
          </div>
          <Button onClick={() => setShowAddDialog(true)} className="bg-amber-500 text-zinc-950 hover:bg-amber-400 rounded-full px-6" data-testid="add-signatory-btn">
            <Plus size={18} className="mr-2" /> {t('vaultSignatories.addSignatory', 'Adicionar Signatário')}
          </Button>
        </div>
      </div>
      ...
      <Card className="bg-zinc-900 border-zinc-800/50">
        <CardContent className="p-6">
          <h3 className="text-xs font-semibold uppercase tracking-[0.2em] text-zinc-500 mb-4 flex items-center gap-2"><Settings size={14} /> {t('vaultSignatories.thresholdConfig', 'Configuração de Threshold')}</h3>
          <div className="grid grid-cols-3 gap-6">
            <div className="space-y-2">
              <Label className="text-zinc-400 text-sm">{t('vaultSignatories.requiredSignatures', 'Assinaturas Necessárias')}</Label>
            ...
            <div className="space-y-2">
              <Label className="text-zinc-400 text-sm">{t('vaultSignatories.timeoutHours', 'Timeout (horas)')}</Label>
            ...
            <div className="flex items-end">
              <Button onClick={saveSettings} disabled={saving} className="bg-amber-500 text-zinc-950 hover:bg-amber-400 rounded-full px-8" data-testid="save-vault-settings-btn">
                <Save size={16} className="mr-2" /> {saving ? t('vaultSignatories.saving', 'A guardar...') : t('vaultSignatories.save', 'Guardar')}
              </Button>
            </div>
          </div>
          {/* Visual threshold */}
          <div className="mt-4 flex items-center gap-2">
            <span className="text-zinc-500 text-xs">{t('vaultSignatories.thresholdLabel', 'Threshold:')}</span>
            ...
            <span className="text-amber-400 text-xs font-mono ml-2">{settings.required_signatures} {t('vaultSignatories.ofLabel', 'de')} {signers.length}</span>
          </div>
        </CardContent>
      </Card>
        ...
        {signatories.length === 0 ? (
          <Card className="bg-zinc-900 border-zinc-800/50">
            <CardContent className="p-12 text-center">
              ...
              <p className="text-zinc-500">{t('vaultSignatories.noSignatories', 'Nenhum signatário configurado')}</p>
              <Button onClick={() => setShowAddDialog(true)} variant="outline" className="mt-4 border-amber-500/30 text-amber-400 rounded-full">
                <Plus size={16} className="mr-1" /> {t('vaultSignatories.addFirst', 'Adicionar primeiro signatário')}
              </Button>
            </CardContent>
          </Card>
        ) : (
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      {s.is_registered && <UserCheck size={14} className="text-emerald-400" title={t('vaultSignatories.registeredOnPlatform', 'Registado na plataforma')} />}
                    </div>
        ...
        <DialogContent className="bg-zinc-900 border-zinc-800 text-white max-w-md" data-testid="add-signatory-dialog">
          ...
          <div className="space-y-4">
            <div className="space-y-1">
              <Label className="text-zinc-400 text-sm">{t('vaultSignatories.labelEmail', 'Email *')}</Label>
              <Input value={newSigner.email} onChange={e => setNewSigner(s => ({ ...s, email: e.target.value }))}
                placeholder={t('vaultSignatories.emailPh', 'email@exemplo.com')} className="bg-zinc-800/50 border-zinc-700/50 text-white rounded-xl" data-testid="signatory-email-input" />
            </div>
            <div className="space-y-1">
              <Label className="text-zinc-400 text-sm">{t('vaultSignatories.labelName', 'Nome')}</Label>
              <Input value={newSigner.name} onChange={e => setNewSigner(s => ({ ...s, name: e.target.value }))}
                placeholder={t('vaultSignatories.namePh', 'Nome do signatário')} className="bg-zinc-800/50 border-zinc-700/50 text-white rounded-xl" data-testid="signatory-name-input" />
            </div>
            <div className="space-y-1">
              <Label className="text-zinc-400 text-sm">{t('vaultSignatories.labelRole', 'Função')}</Label>
            ...
            <Button onClick={addSignatory} className="w-full bg-amber-500 text-zinc-950 hover:bg-amber-400 rounded-full" data-testid="confirm-add-signatory-btn">
              <Plus size={16} className="mr-2" /> {t('vaultSignatories.btnAdd', 'Adicionar')}
            </Button>
          </div>
        </DialogContent>
      </Dialog>
