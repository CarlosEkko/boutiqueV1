...
import PublicSecurityPage from "./pages/legal/SecurityPage";
...
import { KYCStatus } from "./pages/dashboard/kyc";
...
function AppRoutes() {
  return (
    <Routes>
      ...
      <Route path="/legal/status" element={<Navigate to="/system-status" replace />} />
      <Route path="/security" element={<PublicSecurityPage />} />
      <Route path="/help/seguranca" element={<Navigate to="/security" replace />} />
      ...
      >
        ...
        <Route path="my-performance" element={<React.Suspense fallback={<div className="flex items-center justify-center h-64"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-gold-400"></div></div>}><MyPerformance /></React.Suspense>} />
