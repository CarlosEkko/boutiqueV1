...
async def get_market_stats(currency: str = "USD"):
    ...
    import time
    global_market_cap = 0
    global_btc_dominance = 0
    now = time.time()
    if _coingecko_cache["data"] and (now - _coingecko_cache["ts"]) < 300:
    ...
    if not markets:
        return {
            ...
            "total_volume_24h": 0,
            "btc_dominance": round(global_btc_dominance, 2),
            "top_gainer": None,
            "top_loser": None,
            "currency": currency
        }
    
    total_market_cap = global_market_cap or sum(m.get("market_cap") or 0 for m in markets)
    ...
    btc_dominance = global_btc_dominance or ((btc_market_cap / total_market_cap * 100) if btc_market and total_market_cap > 0 else 0)
